...
type ButtonProps = {
  ...
  onClick?: () => void;
  variant?: 'primary' | 'rounded' | 'danger';
...
const buttonStyles = {
  ...
  danger: 'border border-2 bg-red-500 text-white',
};
...
export default function Button({ children, onClick, className, variant = 'primary', size = 'md', ref, rest }: ButtonProps) {
  return (
    <button
      ref={ref}
      onClick={onClick}
      className={cn('rounded-2xl focus:outline-none active:bg-custom-grey', buttonStyles[variant], buttonSizes[size], className)}
      {...rest}>
      {children}
    </button>
  );
